...
#loadImages函数加载所有支持的图片类型；loadFrames函数给具体的子类来调用、获取图片。
...
class Zombie(pg.sprite.Sprite): 
    ...
    def animation(self):  #animation 函数：每隔指定的 animate_interval 时间会显示图片类型的下一个动作。
...
class ZombieHead(Zombie): #zombie父类
...
class ConeHeadZombie(Zombie): #loadFrames函数加载该种僵尸支持的图片类型。实际上，不同种类僵尸的差异主要就是导入的图片类型的名称会有区别。
    def __init__(self, x, y, head_group):
